...
class Vocabulary:
    @staticmethod
    def in_memory(limit: int = 1000, filter: Callable[[str], bool]|None = None) -> list[str]:
        limit = max(0, limit)
        vocabulary = [
        ...
        words = []
        for word in list(set(vocabulary)):
    ...
    @staticmethod
    def from_file(file_path: str, limit: int = 1000, *, filter: Callable[[str], bool]|None = None) -> list[str]:
        reservoir = []
        with open(file_path, 'r') as file:
            for i, line in enumerate(file):
                word = line.strip().upper()
                if filter is not None and not filter(word):
                    continue
...
def main():
    ...
    vocabulary_file_path = './assets/words_alpha.txt'
    vocabulary_limit = 1000
    vocabulary = Vocabulary.from_file(vocabulary_file_path, vocabulary_limit, filter=lambda word: len(word) >= 3)
